...
Begin VB.Form frmIPBillPayments 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.ComboBox cmbBank 
         ...
         ForeColor       =   &H00000080&
         Height          =   315
         ItemData        =   "frmIPBillPayments.frx":0442
         Left            =   870
         List            =   "frmIPBillPayments.frx":045E
         TabIndex        =   15
...
If cmbBillNo.Text = "" Then
    Exit Sub
End If
Set rs = con.Execute("Select * from Bill where BillId=" & cmbBillNo.ItemData(cmbBillNo.ListIndex))
If rs.EOF = True Then
    rs.Close
Else
    ...
    Set rs1 = con.Execute("Select count(*) from BillDetails where BillId=" & cmbBillNo.ItemData(cmbBillNo.ListIndex))
    ...
    rs.Close
End If
Set rs = con.Execute("Select * from BillPayments where BillId=" & cmbBillNo.ItemData(cmbBillNo.ListIndex))
If rs.EOF = True Then
    rs.Close
    txtPaidAmt.Text = "0"
...
End If
RowNo = MFG.Rows - 1
Set rs = con.Execute("Select CustomerAmt from CustomerAdvance where pcode =" & cmbCustName.Text)
If rs.EOF = True Then
    rs.Close
...
End If
cmbBillNo.Clear
Set rs = con.Execute("Select BillNo,BillId from Bill where pcode =" & cmbCustName.Text)
If rs.EOF = True Then
    rs.Close
Else
    Do While rs.EOF = False
...
If txtPayingAmt.Text = "" Then
    ...
    Exit Sub
End If
Set rs = con.Execute("Select Max(BillPaymentId) from BillPayments")
...
If MsgBox("Confirm To Save Bill Information ?", vbQuestion + vbYesNo) = vbYes Then
    con.BeginTrans
    If optCash.Value = True Then
        con.Execute ("Insert into BillPayments values(" & BillPayId & "," & cmbBillNo.ItemData(cmbBillNo.ListIndex) & "," & Val(txtPayingAmt.Text) + (Val(txtCustomerAdv.Text) - Val(txtBalAdv.Text)) & ",'" & Format(dtpPayDate.Value, "mm/dd/yy") & "','" & str & "',Null,Null,Null)")
    ...
    Else
        If txtDDNo.Text = "" Or cmbBank.Text = "" Then
            ...
            Exit Sub
        End If
        con.Execute ("Insert into BillPayments values(" & BillPayId & "," & cmbBillNo.ItemData(cmbBillNo.ListIndex) & "," & Val(txtPayingAmt.Text) + (Val(txtCustomerAdv.Text) - Val(txtBalAdv.Text)) & ",'" & Format(dtpPayDate.Value, "mm/dd/yy") & "','" & str & "','" & txtDDNo.Text & "','" & Format(dtpDDDate.Value, "mm/dd/yy") & "','" & cmbBank.Text & "')")
        MFG.TextMatrix(RowNo, 0) = RowNo
        MFG.TextMatrix(RowNo, 1) = Val(txtPayingAmt.Text) + (Val(txtCustomerAdv.Text) - Val(txtBalAdv.Text))
        MFG.TextMatrix(RowNo, 2) = Format(dtpPayDate.Value, "dd-MMM-yyyy")
        MFG.TextMatrix(RowNo, 3) = str
        ...
        MFG.Rows = MFG.Rows + 1
    End If
    con.Execute ("Update CustomerAdvance Set CustomerAmt=" & Val(txtBalAdv.Text) & " where pcode =" & cmbCustName.Text)
    con.CommitTrans
    Call Txt_Clear
    txtPayingAmt.SetFocus
End If
...
txtBillStatus.Text = ""
txtDDNo.Text = ""
Set rs = con.Execute("Select CustomerAmt from CustomerAdvance where pcode =" & cmbCustName.ItemData(cmbCustName.ListIndex))
If rs.EOF = True Then
    rs.Close
    txtCustomerAdv.Text = "0"
Else
...
i = 0
cmbCustName.Clear
Set rs = con.Execute("Select Name,pcode from INPATIENTS where pcode in (Select Distinct pcode from Bill)")
If rs.EOF = True Then
    rs.Close
Else
    Do While rs.EOF = False
